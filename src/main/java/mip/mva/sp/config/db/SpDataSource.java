...
//						connection.close();
//					} catch (SQLException e) {
//						LOGGER.error("Memory Database CREATE TABLE Connection Close Error!", e);
//					}
//				}
//			} else {
			
			LOGGER.debug(configBean.getVerifyConfig().getDb().getDriverClassName());
			LOGGER.debug(configBean.getVerifyConfig().getDb().getUrl());
			LOGGER.debug(configBean.getVerifyConfig().getDb().getUsername());
			LOGGER.debug(configBean.getVerifyConfig().getDb().getPassword());
